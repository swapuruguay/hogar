...
class Contribuyentes {
  ...
  async getContribuyentes(where, order) {
    ...
    let conn = await connection;
    let listContribuyentes = await conn.query(
      `SELECT * FROM contribuyentes ${cond} ${orden}`
    );
  ...
  async getContribuyente(id) {
    ...
    let conn = await connection;
    let contribuyente = await conn.query(
      `SELECT * FROM contribuyentes WHERE id_contribuyente = ${id}`
    );
    if (!contribuyente) {
      return Promise.reject(new Error("Not found"));
    }
  ...
  async getByCiclo(ciclo) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `SELECT * FROM contribuyentes WHERE id_ciclo_fk = ${ciclo}`
    );
    if (!contris) {
  ...
  async getByCategoria(categoria) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `SELECT * FROM contribuyentes WHERE id_categoria_fk = ${categoria}`
    );
    if (!contris) {
      return Promise.rejetc(new Error("Ocurrio un error"));
    }
  ...
  async deleteContribuyente(idContribuyente) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `UPDATE contribuyentes SET estado = 0 WHERE id_contribuyente = ${idContribuyente}`
    );
    if (!contris) {
      return Promise.reject(new Error("Ocurrio un error"));
    }
  ...
  async getGenerado(mes, anio) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `SELECT * FROM mesesgenerados WHERE anio=${anio} AND mes = ${mes}`
    );
    if (!contris) {
      return Promise.reject(new Error("Ocurrio un error"));
    }
  ...
  async getCuotasPendientes(idContribuyente) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `SELECT * FROM cuotas WHERE estado = 1 AND id_contribuyente_fk = ${idContribuyente}`
    );
    if (!contris) {
      return Promise.reject(new Error("Ocurrio un error"));
    }
  ...
  async getCuotas(mes, anio) {
    ...
    let conn = await connection;
    let contris = await conn.query(
      `SELECT * FROM cuotas WHERE mes = ${mes} AND anio = ${anio}`
    );
    if (!contris) {
      return Promise.reject(new Error("Ocurrio un error"));
    }
  ...
  async generarCli(lista) {
    ...
    let conn = await connection;
    let sql =
      "INSERT INTO contribuyentes (nombre, apellido, domicilio, id_categoria_fk, id_ciclo_fk, mes_pago, estado, fecha_alta, telefono) VALUES ?";
  ...
  async generarCuotas(lista) {
    ...
    let conn = await connection;
    let sql =
      "INSERT INTO cuotas (id_contribuyente_fk, mes, anio, importe, estado, fecha_emision) VALUES ?";
    let result = await conn.query(sql, [lista]);
    if (!result) {
      return Promise.reject(new Error("Ocurrio un error"));
    }
