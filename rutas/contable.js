...
router.get("/recibo/:id", ensureAuth, async (req, res) => {
  ...
  let db = new Bd();
  let movimiento = (
    await db.getMovimientos(` WHERE id_movimiento = ${req.params.id}`)
  )[0];
  db.disconnect();
  let fecha =
  ...
  doc
    .font("fonts/cabin/Cabin-Regular.ttf", 18)
    .fontSize(25)
  ...
  doc.moveDown()
    .text(`Recibí de Hogar de Ancianos de Lascano la suma de $ ${Math.abs(
    movimiento.importe
  )} (${letras(Math.abs(movimiento.importe))})
